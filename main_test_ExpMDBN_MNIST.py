...
if visible_types[0]=="Bernoulli":  
    ...
    reinit_a_use_data_stat=[True,False]
    fix_a_view=[False,False]
elif visible_types[0]=="NegativeBinomial":
    # normalization method
    ...
    maxiter_train=2000
    change_every_many_iters=100
    init_chain_time=10
    visible_type_fixed_param=[100,1]
    reinit_a_use_data_stat=[True,False]
    ...
    visible_type_fixed_param=[0,1]
    reinit_a_use_data_stat=[True,False]
elif visible_types[0]=="Gamma": # Gamma distribution does not work for this data
    # normalize data
    ...
    visible_type_fixed_param=[0,1]
    reinit_a_use_data_stat=[True,False]
elif visible_types[0]=="Multinomial":
    # normalize samples
    normalization_method="samecount"
    count=1000
    # parameter setting
    learn_rate_a_pretrain=[0.001,0.1,0.05]
    learn_rate_b_pretrain=[0.001,0.1,0.05]
    learn_rate_W_pretrain=[0.001,0.1,0.05]
    # for deep
    learn_rate_a_train=[0.001,0.05,0.01]
    learn_rate_b_train=[0.001,0.05,0.01]
    learn_rate_W_train=[0.001,0.05,0.01]
    # for shallow
    #learn_rate_a_train=[0.0005,0.05,0.0005]
    #learn_rate_b_train=[0.0005,0.05,0.0005]
    #learn_rate_W_train=[0.0005,0.05,0.0005]
    change_rate_pretrain=0.9
    change_rate_train=0.8
    reg_lambda_a=0
    reg_alpha_a=1
    reg_lambda_b=0
    reg_alpha_b=1
    ...
    pcdk=20
    NS=100
    maxiter_pretrain=8800
    maxiter_train=8800
    change_every_many_iters=110
    init_chain_time=100
    visible_type_fixed_param=[count,1]
    reinit_a_use_data_stat=[False,False]
    fix_a_view=[False,False]
...
shutil.copy(workdir+"main_test_ExpMDBN_MNIST.py", dir_save)
