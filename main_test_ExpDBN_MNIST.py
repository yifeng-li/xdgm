...
if visible_type=="Bernoulli":  
    ...
    if hidden_type=="Bernoulli":
        ...
        sumout="auto"
        learn_rate_a_pretrain=0.1
        learn_rate_b_pretrain=[0.1,0.01] 
        learn_rate_W_pretrain=[0.01,0.01]
        learn_rate_a_train=0.03
        learn_rate_b_train=[0.03,0.003] # can be a list
        learn_rate_W_train=[0.003,0.003] # can be a list
        reg_lambda_a=0#0.1
        reg_alpha_a=1
    ...
    pcdk=20
    NS=100
    maxiter_pretrain=220#6600
    maxiter_train=110#4400
    change_rate_pretrain=0.9
    change_rate_train=0.9
    change_every_many_iters=11#110
    init_chain_time=10#100
    visible_type_fixed_param=100
    reinit_a_use_data_stat=True
    ...
    normalization_method="tfidf"
    # parameter setting
    learn_rate_a_pretrain=0.0001
    learn_rate_b_pretrain=[0.0001,0.01]
    learn_rate_W_pretrain=[0.0001,0.01]
    learn_rate_a_train=0.00003
    learn_rate_b_train=[0.00003,0.003]
    learn_rate_W_train=[0.00003,0.003]
    reg_lambda_a=0
    reg_alpha_a=[0,0]
    reg_lambda_b=0
    reg_alpha_b=0
    ...
    change_every_many_iters=110
    init_chain_time=100
    visible_type_fixed_param=2*numpy.pi
    reinit_a_use_data_stat=False
elif visible_type=="Gamma":
    # normalize data
    #train_set_x=train_set_x+2
    #test_set_x=test_set_x+2
    #train_set_x=(train_set_x+1)/10
    #test_set_x=(test_set_x+1)/10
    #train_set_x=train_set_x/10 + 1
    #test_set_x=test_set_x/10 + 1
    normalization_method="None"
    count=200
    # parameter setting
    K=500
    learn_rate_a=[0.1,0.1]
    learn_rate_b=0.1 
    learn_rate_W=0.01
    change_rate=0.8
    reg_lambda_a=[0,0]
    ...
    learn_rate_W_pretrain=[0.001,0.01]
    learn_rate_a_train=0.0003
    learn_rate_b_train=[0.0003,0.003] 
    learn_rate_W_train=[0.0003,0.003]
    change_rate_pretrain=0.9
    change_rate_train=0.9
    reg_lambda_a=0
    reg_alpha_a=1
    ...
    change_every_many_iters=110
    init_chain_time=100
    visible_type_fixed_param=count
    reinit_a_use_data_stat=False
...
shutil.copy(workdir+"main_test_ExpDBN_MNIST.py", dir_save)
...
sampling_NS=100
sampling_pcdk=1000
Xg,XMg=model_dbn.generate_x(pcdk=sampling_pcdk, NS=sampling_NS, X0=None, persistent=True, rand_init=True, init=True)
for s in range(sampling_time):
    ...
    print(s)
    cl.plot_image_subplots(dir_save+"/fig_DBN_MNIST_pretrain_generated_samples_randinit_"+str(s)+".pdf", data=sample_set_x_3way, figwidth=6, figheight=6, colormap=None, num_col=10, wspace=0.01, hspace=0.001)
...
for s in range(sampling_time):
    ...
    sample_set_x_3way=numpy.reshape(XMg,newshape=(28,28,100))
    print(s)
    cl.plot_image_subplots(dir_save+"/fig_DBN_MNIST_finetune_generated_samples_randinit_"+str(s)+".pdf", data=sample_set_x_3way, figwidth=6, figheight=6, colormap=None, num_col=10, wspace=0.01, hspace=0.001)
