...
class deep_belief_net:
    def __init__(self, features=None, M=None, K=None, visible_type="Bernoulli", visible_type_fixed_param=1, hidden_type="Bernoulli", hidden_type_fixed_param=0, if_fix_vis_bias=False, a=None, fix_a_log_ind=None, tol_poisson_max=8, rng=numpy.random.RandomState(100)):
        ...
        self.M=M
        self.K=K
        self.NK=len(K) # number of hidden layers, K>=2? When K==1, DBN is a RBM, when K==0, DBN is nothing
        ...
        self.a=[] # generative
        self.b=[] # generative of length L
        self.W=[] # generative, of length L
        self.br=[] # recognition, of length L, but br[L] will not be used
        self.Wr=[] # recognition, of length L, but Wr[L] will not be used
        self.rbms=[]
        self.rng=rng
        self.logZ=0
        self.logZ_std=0
    ...
    def pretrain(self, X=None, batch_size=10, pcdk=20, NS=20 ,maxiter=100, learn_rate_a=0.01, learn_rate_b=0.01, learn_rate_W=0.01, change_rate=0.8, adjust_change_rate_at=None, adjust_coef=1.02, change_every_many_iters=10, init_chain_time=100, train_subset_size_for_compute_error=100, valid_subset_size_for_compute_error=100, track_reconstruct_error=True, track_free_energy=True, reinit_a_use_data_stat=False, if_plot_error_free_energy=False, dir_save="./", prefix="RBM", figwidth=5, figheight=3): 
        ...
        self.cdk=pcdk # for estimate the lower bound of log-likelihood after pretraining
    ...
    def train(self,X=None, X_validate=None, batch_size=10, cdk=20, maxiter=100, learn_rate_a=0.01, learn_rate_b=0.01, learn_rate_W=0.01, change_rate=0.8, adjust_change_rate_at=None, adjust_coef=1.02,  change_every_many_iters=10, init_chain_time=100, train_subset_size_for_compute_error=100, valid_subset_size_for_compute_error=100, track_reconstruct_error=True, track_free_energy=True, if_plot_error_free_energy=False, dir_save="./", prefix="DBN", figwidth=5, figheight=3):
        """
        Wake-sleep (up-down) algorithm to train DBN.
        Different layers have different learning rate.
        """
        start_time=time.clock()
        print("Start training DBN...")
        ...
        if self.batch_size>self.N:
            self.batch_size=1
        self.cdk=cdk # for CD-k sampling in the top RBM
        ...
        for i in range(self.maxiter):
            ...
            ## wake phase, update generative parameters
            Xbatch=self.sample_minibatch(self.batch_size)
            XbatchMg,Hfantacy,HbatchMg,Hbatchr,HbatchMr,a_hat_gen,b_hat_gen=self.sample_xh_wake(Xbatch) # here, Hfantacy is of length L, but only Hfantacy[self.NK-2] is useful
            self.compute_gradient_wake(Xbatch, XbatchMg, Hbatchr, HbatchMg, Hfantacy)
            
            # the update of generaitve parameters can be here as well
            ## udpate generative parameters
            #self.update_param_wake()
            # update the parameters for RBMs
            #self.update_rbms()
            
            ## sleep phase Xg,XMg,Hg,HMr, update recognition paremeters
            Xfantacy,_,Hfantacy,HfantacyMr=self.sample_xh_sleep(Hfantacy) # pass H^(L-1)
            self.compute_gradient_sleep(Xfantacy,Hfantacy,HfantacyMr)
            
            # Hinton's idea: update all parameters at the end
            # udpate generative parameters
            self.update_param_wake()
            # update the parameters for RBMs
            self.update_rbms()
            ...
            if track_free_energy:
                mfe_train,_,_,_,_,_,_,_=self.estimate_log_likelihood(X=Xbatch, Hr=Hbatchr, HMr=HbatchMr, a_hat_gen=a_hat_gen, b_hat_gen=b_hat_gen, estimate_logZ=False, save=False)
            ...
            if self.X_validate is not None:
                if valid_subset_size_for_compute_error is not None:
                    ...
                    if track_reconstruct_error:
                        rec_error_valid,_,Hr_valid,HMr_valid,a_hat_gen_valid,b_hat_gen_valid=self.compute_reconstruction_error(X0=X_validate_subset, X0RM=None)
                    if track_free_energy:
                        mfe_validate,_,_,_,_,_,_,_=self.estimate_log_likelihood(X=X_validate_subset, Hr=Hr_valid, HMr=HMr_valid, a_hat_gen=a_hat_gen, b_hat_gen=b_hat_gen, estimate_logZ=False, save=False)
                else:
                    if track_reconstruct_error:                  
                        rec_error_valid,_,Hr_valid,HMr_valid,a_hat_gen_valid,b_hat_gen_valid=self.compute_reconstruction_error(X0=self.X_validate, X0RM=None)
                    if track_free_energy:
                        mfe_validate,_,_,_,_,_,_,_=self.estimate_log_likelihood(X=X_validate, Hr=Hr_valid, HMr=HMr_valid, a_hat_gen=a_hat_gen, b_hat_gen=b_hat_gen, estimate_logZ=False, save=False)
    ...
    def sample_xh_wake(self, X, compute_HMg=True, cdk=None):
        """
        Use the recognition parameters and the top RBM to sample hidden states.
        """
        # sample h
        ...
        Hfantacy=[None]*self.NK # sampled by the top RBM, only Hfantacy[NK-2 : NK-1] is used here, the rest are None
        for nk in range(self.NK-1):
            if nk==0:
                if self.visible_type=="Multinoulli":
                    b_hat_nk=self.br[nk]
                    for m in range(self.M):
        ...
        # CD sampling in the top RBM
        # Hr[NK-1] stores "Hbatch" in the top RBM, Hfantacy[NK-2]: "XS", Hfantacy[NK-1]: "HS"
        toprbm=self.rbms[self.NK-1]
        #Hr[self.NK-1],HMr[self.NK-1]=toprbm.sample_h_given_x(Hr[self.NK-2])
        #Hfantacy[self.NK-2],Hfantacy[self.NK-1],_,_,_=toprbm.pcd_sampling(pcdk=self.cdk, X0=Hr[self.NK-2], persistent=False)
        Hr[self.NK-1],HMr[self.NK-1]=toprbm.sample_h_given_x(Hr[self.NK-2])
        Hfantacy[self.NK-2],_,_=toprbm.sample_x_given_h(Hr[self.NK-1])
        if cdk is None:
            cdk=self.cdk-1
        for k in range(cdk):
            Hfantacy[self.NK-1],_=toprbm.sample_h_given_x(Hfantacy[self.NK-2])
            Hfantacy[self.NK-2],_,_=toprbm.sample_x_given_h(Hfantacy[self.NK-1])    
        ...
        return XMg,Hfantacy,HMg,Hr,HMr,a_hat_gen,b_hat_gen
    ...
    def compute_posterior_bias_use_recognition_param(self, X, H):
        ...
        # b_hat[self.NK-1] is always None
        return b_hat
    ...
    def compute_posterior_bias_use_generative_param(self, H, a_hat_only=False):
        ...
        # the last one is to compute b_hat^L for the top RBM, this is used in the estimate the lower bound of log-likelihood 
        b_hat[self.NK-1]=self.b[self.NK-1] + numpy.dot(self.W[self.NK-1].transpose(), H[self.NK-2])
            
        return a_hat,b_hat
        
        
    def compute_gradient_wake(self,Xbatch, XbatchMg, Hbatchr, HbatchMg, Hfantacy):
        """
        ...
        # top RBM
        toprbm=self.rbms[self.NK-1]
        toprbm.compute_gradient(Hbatchr[self.NK-2],Hbatchr[self.NK-1],Hfantacy[self.NK-2],Hfantacy[self.NK-1])
        grad_b[self.NK-1]=toprbm.grad_b
        grad_W[self.NK-1]=toprbm.grad_W
    ...
    def sample_xh_sleep(self, Hg):
        """
        Use the generative parameters to sample hidden states and visible states.
        """
        #Hg=[None]*self.NK
        HMg=[None]*self.NK
        for nk in range(self.NK-3,-1,-1):
            b_hat_nk=self.b[nk] + numpy.dot(self.W[nk+1],Hg[nk+1])
            Hg[nk],HMg[nk]=self.sample_h_given_b_hat(b_hat=b_hat_nk, hidden_type=self.hidden_type[nk], hidden_type_fixed_param=self.hidden_type_fixed_param[nk], hidden_value_or_meanfield="value")
    ...
    def generate_x(self, pcdk=1000, NS=100, X0=None, persistent=True, rand_init=True, init=False):
        
        if X0 is not None:
            _,Hfantacy,_,_,_,_,_=self.sample_xh_wake(X0, compute_HMg=False)
            X0_toprbm=Hfantacy[-2]
        else:
            X0_toprbm=None
        Hg=[None]*self.NK        
        toprbm=self.rbms[self.NK-1] # should pass by reference
        X_toprbm,XM_toprbm,H_toprbm,HM_toprbm,_=toprbm.pcd_sampling(pcdk=pcdk, NS=NS, X0=X0_toprbm, persistent=persistent, rand_init=rand_init, init=init)
        Hg[self.NK-1]=H_toprbm
        Hg[self.NK-2]=X_toprbm
        
        Xg,XMg,_,_=self.sample_xh_sleep(Hg=Hg)
...
#                    self.br[nk]=self.br[nk] - self.learn_rate_b[nk] * self.grad_br[nk]
    ...
    def estimate_log_likelihood(self, X=None, Hr=None, HMr=None, a_hat_gen=None, b_hat_gen=None, estimate_logZ=True, base_rate_type="prior", beta=None, step_base=0.999, T=10000, stepdist="even", S=100, save=True, dir_save="./", prefix="DBN"):
        """
        Estimate the lower bound of the log likelihood for DBN.
        """
        if estimate_logZ:
            print("I am estimating the (lower bound) of the log-likelihood...")
        #else:
        #    print "I am estimating the free energy ..."
        if X is None:
            X=self.X
        if Hr is None:
            _,_,_,Hr,HMr,a_hat_gen,b_hat_gen=self.sample_xh_wake(X,compute_HMg=False)
            
        # estimate E_q[log p*(x,h^(1),...,h^(L-1))]
        mean_expect_log_p_star,all_expect_log_p_star=self.compute_Eq_log_p_star_xh(X, HMr, a_hat_gen, b_hat_gen)
        ...
        if estimate_logZ:
            toprbm=self.rbms[self.NK-1]
            self.logZ,self.logZ_std,logws,log_ratio_AIS_mean,log_ratio_AIS_std=toprbm.estimate_logZ(base_rate_type=base_rate_type, beta=beta, step=step_base, T=T, stepdist=stepdist, S=S)

        # estimate H(q)
        mean_entropy,all_entropy=self.compute_entropy(HMr)
        
        # compute log-likelihood 
        loglh_mean = mean_expect_log_p_star - self.logZ + mean_entropy
        
        loglh_all = all_expect_log_p_star - self.logZ + all_entropy
        
        if estimate_logZ:
            print("log-likelihood:{0}, logZ:{1}, logZ_std:{2}, log_p_star: {3}, entropy: {4}".format(loglh_mean, self.logZ, self.logZ_std, mean_expect_log_p_star, mean_entropy))
        
        # save results
        if save:
            result=numpy.zeros(shape=(5,2),dtype=object)
            result[:,0]=numpy.array(["log-likelihood","logZ","logZ_std","log_p_star","entropy"])
            result[:,1]=numpy.array([loglh_mean,self.logZ,self.logZ_std,mean_expect_log_p_star,mean_entropy])
            filename=dir_save + prefix + "_estimated_log_likelihood.txt"
            numpy.savetxt(filename, result, delimiter="\t", fmt="%s")
        
        return loglh_mean,self.logZ,self.logZ_std,mean_expect_log_p_star,mean_entropy,loglh_all,all_expect_log_p_star,all_entropy
        

    def compute_Eq_log_p_star_xh(self, X, HMr, a_hat_gen, b_hat_gen):
        """
        Estimate estimate E_q[log p*(x,h^(1),...,h^(L-1))].
        """
        ...
        # add for the L-1 layer
        z = z + rbm.zeta(self.b[self.NK-2], HMr[self.NK-2], fixed_param=self.hidden_type_fixed_param[self.NK-2], distribution=self.hidden_type[self.NK-2])
        logPar = logPar + rbm.A(b_hat_gen[self.NK-1], fixed_param=self.hidden_type_fixed_param[self.NK-1], distribution=self.hidden_type[self.NK-1])
...
#    def plot_error_free_energy(self, dir_save="./", prefix="DBN", mean_over=5, figwidth=5, figheight=3):
...
#        #plt.close("all")
    ...
    def compute_reconstruction_error(self, X0, X0RM=None, normalize=False):
        """
        Compute the difference between the real sample X0 and the recoverd sample X0RM by mean-field.
        """
        Hr=[None]*self.NK
        if X0RM is None:
            # maybe I can apply wake and sleep here
            X0RM,_,_,Hr,HMr,a_hat_gen,b_hat_gen=self.sample_xh_wake(X0, compute_HMg=False, cdk=None) # HMr,a_hat_gen,b_hat_gen may be used to compute the free energy as well
        else:
            HMr=None
            a_hat_gen=None
            b_hat_gen=None
        if self.visible_type=="Multinoulli":
            ...
            if normalize:
                denominator=numpy.sum(X0,axis=0)
                self.rec_errors=numpy.sum(numpy.abs(X0RM-X0),axis=0)
                self.rec_errors=self.rec_errors/denominator
            else:
                self.rec_errors=numpy.mean(numpy.abs(X0RM-X0),axis=0)
            self.rec_error_mean=numpy.mean(self.rec_errors)
        return self.rec_error_mean,self.rec_errors,Hr,HMr,a_hat_gen,b_hat_gen
    ...
    def compute_entropy(self,HP):
        ...
        entropies=numpy.zeros(num_samples)
        for n in range(num_samples):
            entropies_n=0
            #print "there are {} hidden layers".format(self.NK)
            for l in range(self.NK-1):
...
#            foldername=prefix + "_X"+self.visible_type+":" + str(self.M) + "_H" + "_".join(numpy.array(self.hidden_type)) + ":" + "_".join(numpy.array(self.K,dtype=str)) + "_learnrateabW:" + str(learn_rate_a) + "_" + "_".join(numpy.array(learn_rate_b,dtype=str)) + "_" + "_".join(numpy.array(learn_rate_W,dtype=str)) + "_visfix:" + str(self.visible_type_fixed_param[0]) + "_hidfix:" + "_".join(numpy.array(self.hidden_type_fixed_param,dtype=str)) + "_iter:" + str(maxiter) + "_norm:" + normalization_method + time.strftime("_%Y%b%d%X") + "/"
    ...
    def save_sampling(self, XM, ifsort=True, dir_save="./", prefix="DBN"):
        """
